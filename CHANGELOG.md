## 1.0.55

* Updated map file with new versions:
  - Dart 3.13.4 -> Flutter 3.47.5

## 1.0.54

* Updated map file with new versions:
  - Dart 3.13.3 -> Flutter 3.47.4

## 1.0.53

* Updated map file with new versions:
  - Dart 3.13.3 -> Flutter 3.47.3

## 1.0.52

* Updated map file with new versions:
  - Dart 3.13.2 -> Flutter 3.47.2

## 1.0.51

* Updated map file with new versions:
  - Dart 3.13.1 -> Flutter 3.47.1

## 1.0.50

* Updated map file with new versions:
  - Dart 3.13.0 -> Flutter 3.47.0

## 1.0.49

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.9

## 1.0.48

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.8

## 1.0.47

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.7

## 1.0.46

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.6

## 1.0.45

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.5

## 1.0.44

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.4

## 1.0.43

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.3

## 1.0.42

* Updated map file with new versions:
  - Dart 3.12.2 -> Flutter 3.44.2

## 1.0.41

* Updated map file with new versions:
  - Dart 3.12.1 -> Flutter 3.44.1

## 1.0.40

* Updated map file with new versions:
  - Dart 3.12.0 -> Flutter 3.44.0

## 1.0.39

* Updated map file with new versions:
  - Dart 3.11.5 -> Flutter 3.41.9

## 1.0.38

* Updated map file with new versions:
  - Dart 3.11.5 -> Flutter 3.41.8

## 1.0.37

* Updated map file with new versions:
  - Dart 3.11.5 -> Flutter 3.41.7

## 1.0.36

* Updated map file with new versions:
  - Dart 3.11.4 -> Flutter 3.41.6

## 1.0.35

* Updated map file with new versions:
  - Dart 3.11.3 -> Flutter 3.41.5

## 1.0.34

* Updated map file with new versions:
  - Dart 3.11.1 -> Flutter 3.41.4

## 1.0.33

* Updated map file with new versions:
  - Dart 3.11.1 -> Flutter 3.41.3

## 1.0.32

* Updated map file with new versions:
  - Dart 3.11.0 -> Flutter 3.41.2

## 1.0.31

* Updated map file with new versions:
  - Dart 3.11.0 -> Flutter 3.41.1

## 1.0.30

* Updated map file with new versions:
  - Dart 3.11.0 -> Flutter 3.41.0
  - Dart 3.10.9 -> Flutter 3.38.10

## 1.0.29

* Updated map file with new versions:
  - Dart 3.10.8 -> Flutter 3.38.9

## 1.0.28

* Updated map file with new versions:
  - Dart 3.10.7 -> Flutter 3.38.8

## 1.0.27

* Updated map file with new versions:
  - Dart 3.10.7 -> Flutter 3.38.7

## 1.0.26

* Updated map file with new versions:
  - Dart 3.10.7 -> Flutter 3.38.6

## 1.0.25

* Updated map file with new versions:
  - Dart 3.10.4 -> Flutter 3.38.5

## 1.0.24

* Updated map file with new versions:
  - Dart 3.10.3 -> Flutter 3.38.4

## 1.0.23

* Updated map file with new versions:
  - Dart 3.10.1 -> Flutter 3.38.3

## 1.0.22

* Updated map file with new versions:
  - Dart 3.10.0 -> Flutter 3.38.2

## 1.0.21

* Updated map file with new versions:
  - Dart 3.10.0 -> Flutter 3.38.1

## 1.0.20

* Updated map file with new versions:
  - Dart 3.10.0-290.4.beta -> Flutter 3.38.0

## 1.0.19

* Updated map file with new versions:
  - Dart 3.9.2 -> Flutter 3.35.7

## 1.0.18

* Updated map file with new versions:
  - Dart 3.9.2 -> Flutter 3.35.6

## 1.0.17

* Updated map file with new versions:
  - Dart 3.9.2 -> Flutter 3.35.5

## 1.0.16

* Updated map file with new versions:
  - Dart 3.9.2 -> Flutter 3.35.4

## 1.0.15

* Updated map file with new versions:
  - Dart 3.9.2 -> Flutter 3.35.3

## 1.0.14

* Updated map file with new versions:
  - Dart 3.9.0 -> Flutter 3.35.2

## 1.0.13

* Updated map file with new versions:
  - Dart 3.9.0 -> Flutter 3.35.1

## 1.0.12

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.8

## 1.0.11

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.7

## 1.0.10

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.6

## 1.0.9

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.5

## 1.0.8

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.4

## 1.0.7

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.3

## 1.0.6

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.2

## 1.0.5

* Updated map file with new versions:
  - Dart 3.8.1 -> Flutter 3.32.1

## 1.0.4

* Updated map file with new versions:
  - Dart 3.8.0 -> Flutter 3.32.0

## 1.0.3

* Updated map file with new versions:
  - Dart 3.7.2 -> Flutter 3.29.3

## 1.0.2

* Updated map file with new versions:
  - Dart 3.7.2 -> Flutter 3.29.2

## 1.0.1

* Updated map file with new versions:
  - Dart 3.7.0 -> Flutter 3.29.1

## 1.0.0

* Stable release
  
## 0.1.8

* Updated map file with new versions:
  - Dart 3.7.0 -> Flutter 3.29.0

## 0.1.7

* Updated map file with new versions:
  - Dart 3.6.2 -> Flutter 3.27.4

## 0.1.6

* Updated map file with new versions:
  - Dart 3.6.1 -> Flutter 3.27.3

## 0.1.5

* Updated map file with new versions:
  - Dart 3.6.1 -> Flutter 3.27.2

## 0.1.4

* Update README and improve pub.dev score

## 0.1.3

* Updated map file with new versions:
  - Dart 3.6.0 -> Flutter 3.27.1

## 0.1.0

* Initial release
  - Added initial map of supported Dart to Flutter SDK versions
  - Minimal Flutter SDK version is 3.0.0
